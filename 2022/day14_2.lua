...
local function initGrid()
    ...
    for r = yMin, yMax do
        grid[r] = {}
        for c = xMin - (yMax - yMin), xMax + (yMax - yMin) do
            --print("c", c)
            if r == yMax then
                grid[r][c] = "#"
            else
...
local function printGrid()
    for i = 0, #grid do
        local row = grid[i]
        for col = xMin - (yMax - yMin), xMax + (yMax - yMin) do
            --print(col)
            io.write(row[col])
        end
        print()
...
local function moveGrain(start)
    ...
    if grid[start.y][start.x] == "o" then -- if start is already a sand block
        --print("cas 1")
        return 0
    end

    if start.y == yMax - 1 then -- if just over the floor
        --print("cas 2")
        grid[start.y][start.x] = "o"
        return 1
    end

    if grid[start.y + 1][start.x] == "." then -- Down is an free place
        --print("cas 3")
        return moveGrain({ x = start.x, y = start.y + 1 })
    end

    if grid[start.y + 1][start.x - 1] == "." then -- down left is an free place
        --print("cas 4")
        return moveGrain({ x = start.x - 1, y = start.y + 1 })
    end

    if grid[start.y + 1][start.x + 1] == "." then -- down right is a free place
        --print("cas 5")
        return moveGrain({ x = start.x + 1, y = start.y + 1 })
    end
