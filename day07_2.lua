...
local totalSize = 70000000
local usedSize = root.size
local freeSize = totalSize - usedSize
local neededToUpdate = 30000000
local sizeToGetBack = neededToUpdate - freeSize
print(sizeToGetBack, "must be get back")

local minEnough = root.size
for _, dir in pairs(allDirByFullName) do
    if dir.size >= sizeToGetBack and dir.size < minEnough then
        minEnough = dir.size
    end
end

print("Size of minimum directory is ", minEnough)