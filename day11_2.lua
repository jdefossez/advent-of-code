...
local function simulateMonkey(monkey)
    for _, item in ipairs(monkey.items) do
        ...
        monkey.items[_] = nil
        local currentItem = monkey.operation(item)
        local divisible = currentItem % monkey.divisor == 0
        table.insert(allMonkeys[divisible and monkey.monkeyNumberTrue or monkey.monkeyNumberFalse].items,
            currentItem % ppcm)
    end
end

local function simulate()
    for i = 1, 10000 do -- 20 rounds
        for _, monkey in ipairs(allMonkeysSorted) do
